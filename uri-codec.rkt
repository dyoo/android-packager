...
;; Fork of the uri-codec library in Racket; here because the version
;; in Racket has bad performance.  Trying to fold improvements back into mainline.
;; Until then...

(provide form-urlencoded->alist)
...
(define (form-urlencoded->alist str)
  ...
  (if (equal? "" str)
    '()
    (map (lambda (keyval)
           (let-values ([(key value) (split-key-value keyval)])
             (cons (string->symbol (form-urlencoded-decode key))
                   ;; can be #f for no "=..." part
                   (and value (form-urlencoded-decode value)))))
